...
trait DateRangeHelper
{
    ...
    private function thisDayPreviousMonth(): array
    {
        return [
            'name' => 'total',
            'range' => [
                todayFromApp()->subMonth(),
                todayFromApp(),
            ],
            'context' => 'Y'
        ];
    }
    private function previous30Days(): array
    {
        return [
            'name' => 'previous_30_days',
            'range' => [
                todayFromApp()->subDays(60),
                todayFromApp()->subDays(31)
            ],
            'context' => 'D'
        ];
    }
    private function previous15Days(): array
    {
        return [
            'name' => 'previous_15_days',
            'range' => [
                todayFromApp()->subDays(30),
                todayFromApp()->subDays(15)
            ],
            'context' => 'D'
        ];
    }
    private function previous7Days(): array
    {
        return [
            'name' => 'previous_7_days',
            'range' => [
                todayFromApp()->subDays(14),
    ...
    private function nowLastMonth(): array
    {
        return [
            'name' => 'now_last_month',
            'range' => [
                nowFromApp()->subMonth()->startOfMonth(),
                nowFromApp()->subMonth()->setDay(nowFromApp()->day)
            ],
            'context' => 'Y-m-d'
        ];
    }
    ...
    {
        return ['today', 'thisWeek', 'lastWeek', 'thisMonth', 'lastMonth', 'thisYear', 'nowLastMonth',
                'previous7Days', 'previous15Days', 'previous30Days',
            ];
    }
